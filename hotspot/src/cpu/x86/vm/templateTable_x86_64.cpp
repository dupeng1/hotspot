...
// 向栈压入一个整形常量值，代码并没有压栈操作，它把值放入了rax寄存器，因为相比于寄存器，压栈操作开销比较大，模板解释器把rax和xmm0当作栈顶缓存ToS，
// 凡是能用ToS解决的绝不用栈
...
// 由于整数的四则运算和位运算几乎一样，模板解释器统一把它们放到了iop2中进行处理
...
// 将某个局部变量的值增加到指定大小，它的字节码占用三个字节，分别对应iinc、index、const
// 第一个字节表示它是iinc
// 第二个字节表示局部变量在局部变量表中的索引
// 第三个字节表示递增大小
void TemplateTable::iinc() {
  transition(vtos, vtos); // iinc不改变栈的状态，无需ToS缓存
  __ load_signed_byte(rdx, at_bcp(2)); // get constant 获取const
...
void TemplateTable::branch(bool is_jsr, bool is_wide) {
  ...
  if (UseLoopCounter) {
    ...
    // 如果是回边跳转就执行下面的计数操作，否则直接跳转到dispatch处
  ...
  if (UseLoopCounter) {
    ...
    if (UseOnStackReplacement) {
      ...
      // r14: locals pointer
      // r13: bcp
      // rax存放编译结果，为NULL则表示没有编译，否则表示进行了编译，且这次编译的方法即osr nmethod
      ...
      // 跳转到OSR入口，执行编译后的方法，这样一来就成功完成了从低层次执行到高层次执行的转换
...
void TemplateTable::if_icmp(Condition cc) {
  ...
  // 根据条件进行跳转
  __ jcc(j_not(cc), not_taken);
  // 跳转，branch()实现了各种跳转行为，如if_icmp、goto等
  branch(false, false);
  // 不需要跳转
  __ bind(not_taken);
  __ profile_not_taken_branch(rax);
...
void TemplateTable::_new() {
  // new 会在创建对象后向栈顶压入对象引用，所以ToS从无编程atos
  transition(vtos, atos);
  ...
  // 如果开启 -XX:+UseTLAB，则在TLAB中分配对象，否则在Eden区分配对象
  // 如果分配成功则将对象引用放入rax
  if (UseTLAB) {
...
// 根据虚拟机规范，athrow弹出栈顶的对象引用作为异常对象，然后在当前方法寻找该匹配对象类型的异常处理器，如果找到它则清空当前栈，重新压入异常对象，最后跳转到
// 异常处理器，就像没有发生异常一样；
// 如果没有找到异常处理器，则踏出当前栈帧并恢复到调用者栈帧，然后在调用者栈帧上继续抛出异常，这样相当于将异常继续传播到调用者
