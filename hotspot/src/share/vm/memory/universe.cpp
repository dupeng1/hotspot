  ...
  { FlagSetting fs(_bootstrapping, true);
    ...
    { MutexLocker mc(Compile_lock);
      ...
      if (!UseSharedSpaces) {
        ...
        // 创建出来的一维数组会被存储到类型为Klass*的_typeArrayKlassObjs，这样可以根据这些一维数组的TypeArrayKlass实例创建出多维数组了
...
// 主要就是依据各个GC相关的 option使用不同类型的 CollectedHeap，
// 也就是 ParallelScavengeHeap。 G1CollectedHeap和 GenCollectedHeap。
// 不同类型的CollectedHeap有不同的 CollectorPolicy。
...
jint Universe::initialize_heap() {
  ...
  if (UseParallelGC) {  // -XX:+UseParallelGC
    ...
    // 完成策略初始化，initialize_all()是父类GenCollectorPolicy()的虚函数，它调用了三个子初始化虚函数，这三个子初始化过程由GenCollectorPolicy的子类实现。
